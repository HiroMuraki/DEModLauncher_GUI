...
internal static class DEModManagerViewModelExtensions
{
    public static void TipToNewModPack(this DEModManagerViewModel self)
    {
        try
        {
            ...
            {
                DEModPackViewModel modPack = self.NewModPack();
    ...
    public static void TipToRemoveModPack(this DEModManagerViewModel self, DEModPackViewModel modPack)
    {
        MessageBoxResult result = MessageBox.Show($"是否删除模组配置：{modPack.PackName}", "警告",
    ...
    public static void TipToSaveProfile(this DEModManagerViewModel self)
    {
        MessageBoxResult result = MessageBox.Show("是否保存当前模组配置？", "保存配置", MessageBoxButton.YesNo, MessageBoxImage.Question);
        if (result != MessageBoxResult.Yes)
        {
            return;
        }
    ...
    public static void TipToLoadProfile(this DEModManagerViewModel self, string file)
    {
        MessageBoxResult result = MessageBox.Show("此操作将会重新读取模组配置文件，并丢弃当前设置，是否继续？", "重新读取", MessageBoxButton.YesNo, MessageBoxImage.Warning);
        if (result != MessageBoxResult.Yes)
        {
            return;
        }
    ...
    public static void TipToUpdateResource(this DEModManagerViewModel self, DEModResourceViewModel resource)
    {
        var fileDialog = new System.Windows.Forms.OpenFileDialog
        {
            InitialDirectory = _preOpenModDirectory ?? DOOMEternal.ModPacksDirectory,
            Title = $"替换{resource.Path}"
        };

        if (fileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
        {
            _preOpenModDirectory = fileDialog.FileName;
            self.UpdateResource(resource, fileDialog.FileName);
        }
    }

    public static void TipToOpenResourceFile(DEModResourceViewModel resource)
    {
        try
        {
            var p = new Process();
    ...
    {
        MessageBoxResult result = MessageBox.Show("重置模组加载器将会移除由其产生的文件备份与配置文件\n" +
                                     "请在重置前或重置后验证游戏文件完整性\n" +
                                     "是否继续",
                                     "警告",
    ...
    public static void TipToClearUnusedImageFiles(this DEModManagerViewModel self)
    {
        MessageBoxResult result = MessageBox.Show("该操作将会移除被使用的图片文件，是否继续?", "警告", MessageBoxButton.YesNo, MessageBoxImage.Warning);
        if (result == MessageBoxResult.Yes)
        {
            try
            {
    ...
    public static void TipToClearUnusedModFiles(this DEModManagerViewModel self)
    {
        MessageBoxResult result = MessageBox.Show("该操作将会移除被使用的模组文件，是否继续?", "警告", MessageBoxButton.YesNo, MessageBoxImage.Warning);
        if (result == MessageBoxResult.Yes)
        {
            try
            {
    ...
    public static async Task<bool> TipToStartGame(this DEModManagerViewModel self, StartMode startMode)
    {
        if (startMode != StartMode.StartOnly)
        {
            ...
            // 弹出提示窗口，避免误操作
            MessageBoxResult result = MessageBox.Show($"加载模组将需要一定时间，在此期间请勿关闭本程序。是否继续?",
