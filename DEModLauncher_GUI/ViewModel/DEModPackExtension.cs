...
internal static class DEModPackExtension
{
    public static void OpenWithEditor(this DEModPackViewModel self)
    {
        ...
        if (setter.ShowDialog() == true)
        {
            try
            {
                ...
                {
                    foreach (DEModPackViewModel modPack in DEModManagerViewModel.Instance.ModPacks)
                    {
    ...
    public static void Toggle(this DEModPackViewModel self)
    {
    ...
    public static void TipToCheckModConfliction(this DEModPackViewModel self)
    {
        var sb = new StringBuilder();
        try
        {
            ModPackConflictInfo checkResult = self.GetConflictionInfo();
            string title = "";
    ...
    public static void TipToNewResources(this DEModPackViewModel self)
    {
        ...
        if (ofd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
        {
            ...
            try
            {
                DEModResourceViewModel[] addedRes = self.NewResources(ofd.FileNames, out string[] errors);
    ...
    public static void TipToNewResourcesReference(this DEModPackViewModel self)
    {
        try
        {
            IEnumerable<DEModPackViewModel> allowedModPack = from i in DEModManagerViewModel.Instance.ModPacks
            ...
            {
                foreach (DEModPackViewModel selectedMod in selector.SelectedModPacks)
                {
                    foreach (DEModResourceViewModel item in selectedMod.Resources)
                    {
                        try
                        {
    ...
    public static void TipToInsertResources(this DEModPackViewModel self, int index, IEnumerable<string> fileList)
    {
    ...
    public static void TipToExportMergedResource(this DEModPackViewModel self)
    {
        ...
        if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
        {
            try
            {
                ...
                try
                {
                    ...
                    foreach (DEModResourceViewModel resource in self.Resources)
                    {
